...
signed int LT24_window_background(unsigned short colour,unsigned int x,unsigned int y,unsigned int w_width,unsigned int w_height)
{
    signed int status;
    unsigned int idx;
    ...
    ResetWDT();
    //Define window as entire display (LT24_setWindow will check if we are initialised).
    status = LT24_setWindow(x, y, w_width, w_height);
    if (status != LT24_SUCCESS) return status;
    //Loop through each pixel in the window writing the required colour
    for(idx=0;idx<(w_width*w_height);idx++)
    {
        LT24_write(true, colour);
    }
    //And done.
