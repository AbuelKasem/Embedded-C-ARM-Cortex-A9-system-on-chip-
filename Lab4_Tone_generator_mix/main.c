...
int main(void) {
    ...
    //timer
    int i=0;
    double f_out[25]= { 262, 262, 294, 262, 349, 330, 262, 262, 294, 262, 392,349, 262, 262, 523, 440, 349, 330, 294, 466, 466, 440, 349, 392, 349};
    //Variables
    //Phase Accumulator
   ...
   // inc1   = 440.0 * PI2 / F_SAMPLE;  // Calculate the phase increment based on desired frequency - e.g. 440Hz
    ampl  = (5*8388608.0);               // Pick desired amplitude (e.g. 2^23). WARNING: If too high = deafening!
    inc2   = 880.0 * PI2 / F_SAMPLE;
    phase1 = 0.0;
    phase2 = 0.0;
    ...
    *private_timer_control   = (0<< 8) | (0 << 2) | (1 << 1) | (1 << 0); //dis. auto ,start
    while (1) {
        ...
        if ((fifospace_ptr[2] > 0) && (fifospace_ptr[3] > 0)) {
            ...
            while ((phase1 >= PI2) ||(phase2 >= PI2)) {
                phase1 = phase1 - PI2;
                phase2 = phase2 - PI2;
            }
            // Calculate next sample of the output tone.
            audio_sample1 = (signed int)( ampl * sin( phase1 ) );
            audio_sample2 = (signed int) (( ampl * sin( phase1 ) )+ (ampl * sin( phase2 )) );
